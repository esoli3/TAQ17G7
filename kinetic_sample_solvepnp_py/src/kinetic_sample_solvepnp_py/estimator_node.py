...
# # Target A
# targetA1_cascade = cv2.CascadeClassifier('/home/pi1/catkin_ws/src/kinetic_sample_packages/kinetic_sample_image_sub_py/src/kinetic_sample_image_sub_py/targetA/cascade1.xml')
# targetA2_cascade = cv2.CascadeClassifier('/home/pi1/catkin_ws/src/kinetic_sample_packages/kinetic_sample_image_sub_py/src/kinetic_sample_image_sub_py/targetA/cascade3.xml')
...
# #										(-r, -r, 0.0)])
		...
		pubA_pose = rospy.Publisher("/targetA", Vector3, queue_size=2)
		pubB_pose = rospy.Publisher("/targetB", Vector3, queue_size=2)
		pubC_pose = rospy.Publisher("/targetC", Vector3, queue_size=2)
...
#		self.pose_out.header.frame_id = msg_in.header.frame_id
	...
	def callback_img(self, msg_in):
		...
		# if self.got_camera_info:
			...
			# # Target A 
			# circlesA1 = targetA1_cascade.detectMultiScale(gray, 1.1, 3)
			# circlesA2 = targetA2_cascade.detectMultiScale(gray, 1.1, 3)
			...
			# if circlesA1 is not None:
				...
				# for c in circlesA1:
					# if not have_first_circle:
						...
						# messageA = Vector3()
						# messageA.x = (c[0] + c[2]/2)* 0.00236
						# messageA.y= (c[1] + c[3]/2)* 0.00236
						# messageA.z = 1.5

						# pubA_pose.publish(messageA)

						# #Draw the circle
						
						# for (x,y,w,h) in circlesA1:
							# cv2.rectangle(cv_image,(x,y),(x+w,y+h),(255,255,0),2)


			# if circlesA2 is not None:
				# have_first_circle = False
			...
			# if circlesB1 is not None:
				...
				# for c in circlesB1:
					# if not have_first_circle:
						...
						# messageB = Vector3()
						# messageB.x = (c[0] + c[2]/2)* 0.00236
						# messageB.y= (c[1] + c[3]/2)* 0.00236
						# messageB.z = 1.5

						# pubB_pose.publish(messageB)
			...
			# if circlesC1 is not None:
				...
				# for c in circlesC1:
					# if not have_first_circle:
						...
						# messageC = Vector3()
						# messageC.x = (c[0] + c[2]/2)* 0.00236
						# messageC.y= (c[1] + c[3]/2)* 0.00236
						# messageC.z = 1.5

						# pubC_pose.publish(messageC)
