...
class MavrosGuider {
    private:
        ...
        // ros::Subscriber target_detect_;     //subscriber to IMP
        // ros::Publisher target_position_;      // publisher to GCS for target
        ros::Publisher servo_pub_;     //publisher to servo 
    ...
    public:
        MavrosGuider() :
            ...
            // target_detect_ = nh_.subscribe<>();     sub to IMP
            //target_position_ = nh_.advertise<std_msgs::String>("target_location", 10);   //pub to GCS 
            servo_pub_ = nh_.advertise<std_msgs::bool>("servo_position", 10);         //pub to Servo
            ...
            void pose_cb( const geometry_msgs::PoseStamped msg_in)
            {
                ...
                {
                    // publish servo face down
                    camera_down = true;
                    servo_position.publish(camera_down); 
                }
                ...
                if(target_found == true)
                {
                    ...
                    std_msgs::String target_loc;
                    std::stringstream ss; 
                    ss<<"Target located at: x = [%0.2f], y = [%0.2f], z = [%0.2f]", target_x, target_y, target_z << count; 
                    target_loc.data = ss.str(); 
                    ROS_INFO("%s", target_loc.data.c_str());  
                    ...
                    else if(camera_down == true)
                    {
                        msg_pose_out_.pose.position.z = 1.2; 
                    }
